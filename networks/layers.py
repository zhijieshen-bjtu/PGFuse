...
from .PanoGaborv2 import GaborConv2d
...
#from  https://github.com/houqb/SPNet/blob/master/models/customize.py      
class CELayer(nn.Module):
    def __init__(self, in_channels):
        super(CELayer, self).__init__()
        self.vpool = nn.AdaptiveMaxPool2d((1, None))
        self.hpool = nn.AdaptiveMaxPool2d((None, 1))
        self.fc = nn.Sequential(
    ...
    def forward(self, x):
        b,c,h,w = x.size()
        hp = self.vpool(x).repeat(1,1,h,1)
        vp = self.hpool(x).repeat(1,1,1,w)
        avg_pool = hp+vp
        # 全连接层
...
class CEELayer(nn.Module):
    def __init__(self, channels, height, SE=True):
        ...
        #self.selayer = SELayer(height)
        #self.celayer = CELayer(channels)

        self.res_conv2 = GaborConv2d(channels, height, kernel_size=3, padding=1, bias=False)
        self.res_bn2 = nn.BatchNorm2d(height)
        #self.ln = nn.LayerNorm(channels)
        self.relu = nn.ReLU(inplace=True)
        self.selayer = SELayer(channels)

        self.conv = nn.Conv2d(channels, 256, 1, bias=False)
    ...
    def forward(self, equi_feat, c2e_feat):
        ...
        #print(x.shape)        
        shortcut = self.relu(self.res_bn2(self.res_conv2(x)))
        #shortcutg = self.selayer(shortcut)
        x = x* torch.mean(shortcut, dim = 1, keepdim=True)#self.celayer(shortcut)

        #x = x + c2e_feat #x+c2e_feat+equi_feat        #x = torch.cat([equi_feat, x], 1)
        x = x + self.selayer(c2e_feat)
