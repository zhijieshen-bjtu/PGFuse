...
def main():
    ...
    model_path = os.path.join(load_weights_folder, "sp.pth")
    md = os.path.join(load_weights_folder, "model.pth")
    ...
    test_dataset = dataset(settings.data_path, test_file_list, model_dict['height'], model_dict['width'], is_training=False)
    ...
    print(model_dict['se_in_fusion'])
    print(model_dict['fusion'])
    print(model_dict['layers'])
    ...
    model = Net(model_dict['layers'], 512, 1024,
    ...
    with torch.no_grad():
        for batch_idx, inputs in enumerate(pbar):
            ...
            rcube_inputs = inputs["rnormalized_cube_rgb"].to(device)

            cube_inputs = inputs["normalized_cube_rgb"].to(device)
            
            #model.eval()

            outputs = model(rcube_inputs, cube_inputs)
            ...
            for i in range(gt_depth.shape[0]):
              evaluator.compute_eval_metrics(gt_depth[i:i+1], pred_depth[i:i+1], mask[i:i+1], 10)
