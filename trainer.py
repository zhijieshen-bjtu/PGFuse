...
import datasets
from networks import PGFuse, Equi
from networks.spconv import SphericalGradientLoss, Gradient_Net
...
class Trainer:
    def __init__(self, settings):
        ...
        datasets_dict = {"3d60": datasets.ThreeD60,
                         ...
                         "structure3d": datasets.Structure3D}
        ...
        self.maxmeter = train_dataset.max_depth_meters
        ...
        val_dataset = self.dataset(self.settings.data_path, val_file_list, self.settings.height, self.settings.width,
                                   True, True, True, is_training=False)
        num_train_samplesval = len(val_dataset)
        self.wr = num_train_samplesval%self.settings.batch_size
        ...
        self.sgloss1 = Gradient_Net()
        self.sgloss2 = SphericalGradientLoss()
    ...
    def process_batch(self, inputs):
        ...
        rcube_inputs = inputs["rnormalized_cube_rgb"]

        cube_inputs = inputs["normalized_cube_rgb"]

        outputs = self.model(rcube_inputs, cube_inputs)
        
        #attention_weights = torch.ones_like(inputs["val_mask"], dtype=torch.float32, device=inputs["val_mask"].device)
        
        
        losses["loss"] = self.compute_loss(inputs["gt_depth"],
                                           outputs["pred_depth"],
                                           inputs["val_mask"]) + 0.5*self.sgloss2(inputs["gt_depth"],
                                           outputs["pred_depth"]*inputs["val_mask"].float(),
                                           inputs["val_mask"])
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(pbar):
                ...
                for i in range(gt_depth.shape[0]):
                  self.evaluator.compute_eval_metrics(gt_depth[i:i+1], pred_depth[i:i+1], mask[i:i+1], self.maxmeter)
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(self.wr, self.settings.batch_size)):  # write a maxmimum of four images
    ...
    def save_settings(self):
        ...
        """
        shutil.copy('networks/PanoGaborv2.py', self.log_path)
        shutil.copy('networks/panores.py', self.log_path)
        shutil.copy('networks/pgfuse.py', self.log_path)
        shutil.copy('trainer.py', self.log_path)
        shutil.copy('networks/layers.py', self.log_path)
        shutil.copy('datasets/stanford2d3d.py', self.log_path)
        shutil.copy('datasets/matterport3d.py', self.log_path)
        shutil.copy('datasets/structure3d.py', self.log_path)
    ...
    def save_model(self):
        ...
        save_path_modeldict = os.path.join(save_folder, "{}.pth".format("model"))
        torch.save(self.model.state_dict(), save_path_modeldict)
        save_path_superp = os.path.join(save_folder, "{}.pth".format("sp"))
        to_save = {}
        ...
        torch.save(to_save, save_path_superp)
