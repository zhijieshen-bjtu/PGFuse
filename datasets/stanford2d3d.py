...
class Stanford2D3D(data.Dataset):
    ...
    def __init__(self, root_dir, list_file, height=512, width=1024, disable_color_augmentation=False,
        ...
        if is_training:
            self.rgb_depth_list = 4*self.rgb_depth_list
        self.w = width
        self.h = height
    ...
    def __getitem__(self, idx):
        ...
        rgb_name = rgb_name.replace('rgb_', 'pano/rgb')
        #print(rgb_name)
        rgb = cv2.imread(rgb_name)
        rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)
        rgb = cv2.resize(rgb, dsize=(self.w, self.h), interpolation=cv2.INTER_AREA)

        depth_name = os.path.join(self.root_dir, self.rgb_depth_list[idx][1])
        depth_name = depth_name.replace('depth/', 'pano/depth/')
        gt_depth = cv2.imread(depth_name, -1)
        gt_depth = cv2.resize(gt_depth, dsize=(self.w, self.h), interpolation=cv2.INTER_AREA)
        ...
        #print(inputs["normalized_cube_rgb"].shape)
        ...
        inputs["val_mask"] = ((gt_depth >0) & (gt_depth <= self.max_depth_meters)
                                & ~torch.isnan(gt_depth))#  & mask.bool()
                                
        inputs["gt_depth"] = gt_depth * inputs["val_mask"].float()
        
        #inputs["val_mask"] = inputs["val_mask"]
